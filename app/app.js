...
const carrinhoItensGauge = new promClient.Gauge({
    name: 'ecommerce_carrinho_itens',
    help: 'Total de itens atualmente nos carrinhos'
});

const pedidosStatusGauge = new promClient.Gauge({
    name: 'ecommerce_pedidos_status',
    help: 'Pedidos ativos por status',
    labelNames: ['status']
});

// Helper: atualiza contagem de itens no carrinho
function updateCarrinhoItensGauge() {
    let total = 0;
    for (const userId of Object.keys(carrinhos)) {
        total += carrinhos[userId].items.reduce((s, i) => s + i.quantidade, 0);
    }
    carrinhoItensGauge.set(total);
}

// Helper: atualiza gauge de pedidos por status
function updatePedidosStatusGauge() {
    const contagem = {};
    for (const p of pedidos) {
        contagem[p.status] = (contagem[p.status] || 0) + 1;
    }
    // Zera todos os status conhecidos e atualiza
    ['pendente', 'pago', 'pagamento_falhou', 'enviado', 'entregue', 'cancelado'].forEach(s => {
        pedidosStatusGauge.set({ status: s }, contagem[s] || 0);
    });
}
